...
Runs all test suites for the project using the Taito CLI: client and server unit tests, server
integration/e2e tests, and Playwright E2E tests. The Taito CLI handles the environment and
containers, so there is no need to manage Docker or start services by hand.
